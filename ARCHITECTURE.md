...
### 6.3 失败可见性：两条通道**都**要，只做一半等于没做

分成两条通道的代价是：失败也有两种，而它们**渲染在同一块屏幕上**。用户不会区分「哪条通道死了」，他只会判断「这个界面是不是在骗我」。所以两条通道各自的判死动作刻意不同，但「必须显式」这一条对两者同等适用：

| | 控制通道 | 数据通道 |
| --- | --- | --- |
| 健康度 | `ControlLinkHealth`（10s 心跳，丢 2 拍判死，§2.2 / [bridge-contract.md §1.6](./docs/bridge-contract.md)） | `RuntimeLinkState` + `DataAvailability` 读模型 |
| 判死动作 | **撤下所有原生视图**，官方 Web UI 接管（ADR-0004） | 原生视图**留在屏幕上**，但必须显式说明「数据不可信」，并给可重试入口 |
| 判死后 | 不自动复活（界面不许在两种实现间抖动） | 自动重连（指数退避 + 封顶 + 稳定窗口），数据回来就继续画 |

之所以不能反过来：控制通道死了，原生视图连 props 与几何都拿不到，留着就是一块骗人的界面；数据通道死了，编排还活着，此时退回 Web UI 反而更糟 —— 官方 UI 用的是同一个 runtime，它一样没有数据，只是不会告诉你。

两条推论（[known-gaps.md G-9](./docs/known-gaps.md) 的教训，代价是一个真实的 bug）：

1. **每一条会渲染成「什么都没有」的路径，都必须能回答「我凭什么敢说没有」。** 空列表、空详情、零计数都是**关于数据的断言**，前提是链路可信。判据要住在数据层的读模型里（`DSHClient.dataAvailability`），不许是视图里的 `列表.isEmpty` —— 后者只知道行数，回答不了前提。
2. **`try? … ?? 空值` 是失败可见性的头号敌人。** 它把「读不懂对端的回答」翻译成「你没有数据」，而这两句话在屏幕上恰好长得一样。宽容解码只允许用在**后加的可选字段**上（缺失与默认值同义），绝不允许用在列表主体上：宁可抛错走失败态，也不静默降级成空态。

第三条是真机 dogfood 补上的（[G-13](./docs/known-gaps.md)）：

3. **「失败可见」不止于「连不上要说出来」，还包括「数据过期不能装作现状」。** 一份静止的旧列表和一份实时列表在屏幕上长得一模一样，所以保留旧数据（不清屏）与把旧数据说成现状是两件事 —— `DataAvailability` 因此把 `.populated` 与 `.stale` 分开，`isCurrent` 只对「成功对齐过全量 + 事件流活着」为真。同理，**增量只有在足以重建读模型时才能当真值**：判据是逐字段问「这个值能否只从事件流推出来」，答不上来的（`blank`、工作区归属）就只能把事件当失效信号，回头拉全量。

同一条规律在 TS 侧的形式是 [G-10](./docs/known-gaps.md)：**一个猜出来的地址比没有地址更糟** —— 缺失会触发兜底与诊断，错值会把两者都关掉。凡是「没有依据却给出自信答案」的地方，都要么去问真正知道的那一方，要么闭嘴并让闭嘴可见。
