...
type TokenText = { primary: string; secondary: string; tertiary: string };
type TokenBorder = { subtle: string; strong: string };
type TokenScale = Record<string, number | string>;
...
const EMBEDDED: Record<string, ThemePack> = {
  "studio-dark": {
    id: "studio-dark",
    name: "Studio Dark",
    nameZh: "Studio 深色",
...
const state: {
  ...
  tokenOverride: ThemeTokens | null;
...
function themeSearchDirs(): string[] {
...
function asScale(v: unknown, fallback: TokenScale): TokenScale {
...
function parseTokens(raw: unknown, fallback: ThemeTokens): ThemeTokens {
...
function parsePack(raw: unknown): ThemePack | null {
...
function loadPacksFromDisk(): Map<string, ThemePack> {
...
function resolveTokens(): { pack: ThemePack; tokens: ThemeTokens } {
...
export function tokensToCssVars(tokens: ThemeTokens): Record<string, string> {
  const px = (v: number | string) => (typeof v === "number" ? `${v}px` : String(v));
  const ms = (v: number | string) => (typeof v === "number" ? `${v}ms` : String(v));
...
export function cssVariablesStylesheet(tokens: ThemeTokens): string {
...
function trySchemastery(): {
  ...
  try {
    ...
    const mod = require("@deepseek-ai/schemastery") as { default?: unknown } | unknown;
    ...
    if (Schema && typeof Schema.object === "function") {
      return Schema as ReturnType<typeof trySchemastery> & object;
    }
  } catch {
...
function configIssues(raw: unknown): { message: string }[] {
  if (raw == null) return [];
 ...
 */
function standardSchemaConfig(): unknown {
  return {
    "~standard": {
      version: 1,
      vendor: "dsh-studio",
...
function buildConfigSchema(): unknown {
...
function pickTitle(session: unknown, event: unknown): string {
  const from = (obj: unknown, keys: string[]): string | undefined => {
...
function pickSessionId(session: unknown, event: unknown): string | undefined {
  const from = (obj: unknown): string | undefined => {
    if (!isRecord(obj)) return undefined;
...
function rememberEvent(session: unknown, event: unknown): void {
...
export function startBridge(config: Config): { close: () => Promise<void> } {
...
function wireSessionEvents(ctx: LooseCtx): void {
...
export function apply(ctx: LooseCtx, config?: unknown): void {
